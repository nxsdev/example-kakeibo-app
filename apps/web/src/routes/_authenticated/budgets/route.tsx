  ...
  loader: async ({ context }) => {
    ...
    await Promise.all([
      context.queryClient.ensureQueryData(
        context.orpc.budgets.list.queryOptions({ input: { month: currentMonth } }),
      ),
      context.queryClient.ensureQueryData(
        context.orpc.categories.list.queryOptions({ input: { type: "expense" } }),
      ),
    ]);
    ...
    <div className="mx-auto max-w-2xl space-y-4">
      ...
      <div className="space-y-3">
        {budgets.items.map((budget) => {
          const percentage =
            budget.amount > 0 ? Math.min(Math.round((budget.spent / budget.amount) * 100), 100) : 0;
        ...
        {budgets.items.length === 0 ? (
          <p className="py-8 text-center text-sm text-muted-foreground">予算が設定されていません</p>
        ) : null}
      </div>
    </div>
  );
